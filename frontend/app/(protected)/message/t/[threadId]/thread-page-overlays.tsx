...
import type { CallSummary, ConversationListItem, DirectoryUser, Message, MessageRemovalMode } from "@/types/chat";
...
interface ActiveCallPanelProps {
  ...
  show: boolean;
  activeCallDisplayName: string;
  activeCallStatusLabel: string;
  showActiveCallDuration: boolean;
  activeCallDurationLabel: string;
  activeCallNetworkToneClassName: string;
  activeCallNetworkLabel: string;
  callRemoteStream: MediaStream | null;
  callLocalStream: MediaStream | null;
  isCallCameraOff: boolean;
  isCallMuted: boolean;
  activeCallRemoteVideoRef: RefObject<HTMLVideoElement | null>;
  activeCallLocalVideoRef: RefObject<HTMLVideoElement | null>;
  onEndCall: () => void;
  onToggleMute: () => void;
  onToggleCamera: () => void;
}

export function ThreadActiveCallPanel({
  currentCall,
  show,
  ...
  activeCallNetworkToneClassName,
  activeCallNetworkLabel,
  callRemoteStream,
  callLocalStream,
  isCallCameraOff,
  isCallMuted,
  activeCallRemoteVideoRef,
  activeCallLocalVideoRef,
  onEndCall,
  onToggleMute,
  onToggleCamera,
    ...
    <div className="fixed inset-x-0 bottom-4 z-40 flex justify-center px-4">
      <div className="w-full max-w-4xl overflow-hidden rounded-[28px] border border-slate-200/80 bg-white/95 shadow-2xl backdrop-blur">
        <div className="flex items-center justify-between gap-3 border-b border-slate-200 px-5 py-4">
          ...
          <Button type="button" variant="danger" className="rounded-full" onClick={onEndCall}>
            End call
          </Button>
        </div>
        ...
        {currentCall.call_type === "video" ? (
          <div className="grid gap-3 bg-slate-950 p-4 md:grid-cols-[minmax(0,1fr)_240px]">
            <div className="relative flex min-h-[260px] items-center justify-center overflow-hidden rounded-3xl border border-white/10 bg-slate-900">
              {callRemoteStream ? (
                <video ref={activeCallRemoteVideoRef} autoPlay playsInline className="h-full w-full object-cover" />
              ) : (
            ...
            <div className="flex min-h-[260px] flex-col gap-3">
              <div className="relative flex flex-1 items-center justify-center overflow-hidden rounded-3xl border border-white/10 bg-slate-900">
                {callLocalStream && !isCallCameraOff ? (
                  <video ref={activeCallLocalVideoRef} autoPlay muted playsInline className="h-full w-full object-cover" />
                ) : (
                  <div className="text-center text-white">
              ...
              <div className="rounded-3xl bg-white p-3">
                <div className="grid grid-cols-3 gap-2">
                  <Button type="button" variant={isCallMuted ? "outline" : "secondary"} className="rounded-full" onClick={onToggleMute}>
